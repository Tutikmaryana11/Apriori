<?php
	...
	$aksi = 'mod/returpembelian/act_retur_pembelian.php';
	...
	switch ($act) {
						...
						if(mysql_num_rows($sql_tmp) > 0)
						{
							while ($b = mysql_fetch_assoc($sql_tmp)) {
								...
								echo"<tr style='border-bottom:1px dashed #ccc;'>
									...
									<td><a href='$aksi?mod=returpembelian&act=batal&id=$b[kode_barang]' onclick=\"return confirm('Yakin ingin membatalkan?');\"><i class='fa fa-close w3-tiny w3-text-grey'></i></a></td>
								</tr>";
		...
		case "printout" :
			...
			if(isset($_GET['id']))
			{
				echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
					<h4 style='margin-bottom:0;padding-bottom:0;'>RETUR PENJUALAN</h4>
					<p style='margin-top:0;padding-top:0;'><i>Data Retur Penjualan Barang</i></p>
				</div><br>
				...
				$sqltrans = mysql_query("SELECT * from tb_retur_penjualan r JOIN tb_penjualan p on r.no_transaksi=p.no_transaksi where r.no_transaksi= '$_GET[id]'") or die(mysql_error());
				...
				echo"<table class='w3-table w3-tiny'>
					...
					<tr style='border-bottom:1px dashed #ccc;'>
						<td>Keterangan</td>
						<td>:</td>
						<td><input type='text' name='keterangan' placeholder='Alasan Retur'></td>
					</tr>
				</table>
				...
				$sql = mysql_query("SELECT a.*, b.nama_barang, b.satuan 
									FROM tb_retur_penjualan a LEFT JOIN tb_barang b 
				...
				<p>
					...
					<a href='med.php?mod=returpembelian' class='w3-btn w3-tiny'><i class='fa fa-cart-plus'></i> Transaksi Baru</a>
					<a href='popup/popup.php?mod=cetakbuktiretur&id=$_GET[id]' class='w3-btn w3-dark-grey w3-tiny' target='_blank'><i class='fa fa-print'></i> Cetak Bukti Retur</a>
				</p>";
		...
		case "list":
			echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				<h4 style='margin-bottom:0;padding-bottom:0;'>Retur Pembelian</h4>
				<p style='margin-top:0;padding-top:0;'><i>Data Semua Transaksi Pembelian</i></p>
			</div>";
			...
			echo"<table style='margin-top:12px;'>
				<tr>
					...
					<td align='right'><a href='med.php?mod=returpembelian&act=list' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					</td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$no</td>
							<td><a class='w3-text-blue w3-hover-text-red' href='med.php?mod=returpembelian&act=detail&id=$m[no_faktur]'>$m[no_faktur]</a></td>
							<td>$m[nama_toko]</td>
							<td>$m[tgl_beli]</td>
							<td>$m[nama_kasir]</td>
		...
		case "prosesretur" :
			...
			if(isset($_GET['id']))
			{
				...
				echo"<form action='$aksi?mod=returpembelian&act=simpan' method='POST' class='w3-container'>
				...
				echo"<table class='w3-table w3-tiny'>
					<tr style='border-bottom:1px dashed #ccc;'>
						...
						<td width='10px'>:</td>
						<td><b>$tra[no_transaksi]</b></td>
						<td><input type='hidden' name='no_transaksi' value='$tra[no_transaksi]'></td>
					</tr>
					...
					<tr style='border-bottom:1px dashed #ccc;'>
						...
						<td>:</td>
						<td><b>$tra[nama_pelanggan] / "?><?php echo !empty($tra['kode_pelanggan']) ? $tra['kode_pelanggan'] : "-"; ?><?php echo"</b></td>
						<td><input type='hidden' name='nama_pelanggan' value='$tra[nama_pelanggan]'></td>
					</tr>
					...
					<tr style='border-bottom:1px dashed #ccc;'>
						...
						<td>:</td>
						<td><b>$tra[timestmp]</b></td>
						<td><input type='hidden' name='timestmp' value='$tra[timestmp]'></td>
					</tr>
				...
				while($p = mysql_fetch_assoc($sql))
				{
					...
					echo"<tr>
						<td>$no</td>
						<td>$p[kode_barang];<input type='hidden' name='kode_barang' value='$p[kode_barang]'></td>
						<td>$p[nama_barang];<input type='hidden' name='nama_barang' value='$p[nama_barang]'></td>
						<td>Rp. ".number_format($p['harga'],0)." X $p[qty] $p[satuan];<input type='hidden' name='harga' value='$p[harga]'><input type='hidden' name='qty' value='$p[qty]'></td>

						<td>".number_format($p['disc'],0)."%</td><input type='hidden' name='disc' value='$p[disc]'>
						<td>Rp. ".number_format($sub_total)."</td><input type='hidden' name='subtotal' value='$sub_total'>
						<td><input type='checkbox' name='pilih' value='pilih'></td>
					</tr>";
				...
				<p>
					...
					<a href='med.php?mod=returpenjualan' class='w3-btn w3-tiny'><i class='fa fa-cart-plus'></i> Transaksi Baru</a>
					<a href='popup/popup.php?mod=cetakkwitansi&id=$_GET[id]' class='w3-btn w3-dark-grey w3-tiny' target='_blank'><i class='fa fa-print'></i> Cetak Bukti Retur</a>
				</p>
...
case "detail" :
			...
			if(isset($_GET['id']))
			{
				...
				echo"
				<form action=med.php?mod=returpembelian&act=simpan' method='POST' class='w3-container'>
				<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				...
				echo"<table class='w3-table w3-tiny'>
					<tr style='border-bottom:1px dashed #ccc;'>
						...
						<input type='hidden' name='no_faktur' value='$tra[no_faktur]'>
					</tr>
					...
					<tr style='border-bottom:1px dashed #ccc;'>
						...
						<input type='hidden' name='nama_toko' value='$tra[nama_toko]'>
				...
				<p>
					...
					<input type='submit' name='submit' value='Retur'>
				</p>
				</form>	";
