	...
	switch ($act) {
						...
						if(mysql_num_rows($sql_tmp) > 0)
						{
							while ($b = mysql_fetch_assoc($sql_tmp)) {
								...
								echo"<tr style='border-bottom:1px dashed #ccc;'>
									...
									<td><a href='$aksi?mod=returpenjualan&act=batal&id=$b[kode_barang]' onclick=\"return confirm('Yakin ingin membatalkan?');\"><i class='fa fa-close w3-tiny w3-text-grey'></i></a></td>
								</tr>";
		...
		case "list":
			...
			echo"<table style='margin-top:12px;'>
				<tr>
					...
					<td align='right'><a href='med.php?mod=returpenjualan&act=list' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					</td>
				</tr>
			...
			<table class='w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl'>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							<td>$no</td>
							<td><a class='w3-text-blue w3-hover-text-red' href='med.php?mod=returpenjualan&act=prosesretur&id=$m[no_transaksi]'>$m[no_transaksi]</a></td>
							...
							<td>$m[status]</td>
							<td><a href='$aksi?mod=returpenjualan&act=hapus&id=$m[no_transaksi]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a>
							</td>
		...
		case "prosesretur" :
			...
			if(isset($_GET['id']))
			{
				...
				echo"<form action='$aksi?mod=returpenjualan&act=simpan' method='POST' class='w3-container'>
